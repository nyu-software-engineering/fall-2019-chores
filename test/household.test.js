...
describe('Test household creation validity.', () => {
    ...
    it('test if the dates match', function (done) {
        assert.equal(house.createdOn, household.createdOn, 'last name doesn\'t match');
        done();
    });
...
describe("Household Tests", function() {
	...
	it("test title modification", function(done) {
      	user.setTitle("MyHousehold2");
      	assert.equal(user.getTitle(), "MyHousehold2");
      	done();
    });

    it("test admin modification", function(done) {
      	user.setAdmin(user);
      	assert.equal(user.getAdmin(), user);
      	done();
    });
